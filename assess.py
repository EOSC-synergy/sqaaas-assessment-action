...
def get_credentials_id():
    ...
    try:
         ...
         credentials_id = json.loads(json.dumps(eval(credentials_id)))
    if credentials_id:
        logger.info("credentials present")
    else:
        logger.info("No credentials presented")    
...
def main():
    ...
    only, criteria_intended= get_criteria_only()
    credentials_id= get_credentials_id()
    logger.info("Trigger SQAaaS assessment with creds: %s" % credentials_id)
